...
private:
    bool isValid(vector<vector<char>> &board, int row, int col, int num)
    {
        for (int i = 0; i < 9; i++)
        {
            ...
            int calRow = 3 * (row / 3) + i / 3;
            int calCol = 3 * (col / 3) + i % 3;
            if (board[calRow][calCol] == num)
                return false;
        }
...
private:
    ...
    void helperFunc(vector<int> &candidates, vector<int> tempAns, int idx, int target)
    {
        ...
        for (int i = idx; i < candidates.size(); i++)
        {
            ...
            helperFunc(candidates, tempAns, i + 1, target - candidates[i]);
...
public:
    vector<vector<int>> combinationSum2(vector<int> &candidates, int target)
    {
        ...
        helperFunc(candidates, tempAns, 0, target);
        return output;
    }
};
